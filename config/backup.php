...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 *
                 * Intentionally empty: this platform is event-sourced, so the
                 * database IS the ledger. The scheduled job runs
                 * `backup:run --only-db` (routes/console.php) — file backups
                 * are a deploy-artifact concern, not a data-recovery one.
                 */
                'include' => [],
        ...
        'destination' => [
             ...
             *
             * Defaults to `local` so the job works out of the box; production
             * must set BACKUP_DISK=s3 (plus the AWS_* bucket credentials in
             * config/filesystems.php) so dumps leave the box. See
             * docs/10-OPERATIONS/PRODUCTION_READINESS_CHECKLIST.md §9.
             */
            'disks' => [
    ...
    'notifications' => [
        /*
         * Mail/Slack notification channels are intentionally OFF by default.
         * Alerting goes through the log instead: the package writes backup
         * activity to the default log channel (`log_channel` => null below),
         * and the scheduled jobs in routes/console.php fire Log::critical on
         * failure — which reaches Slack automatically via the LOG stack when
         * LOG_SLACK_WEBHOOK_URL is set (PR #1133). Re-enable 'mail'/'slack'
         * here only if you want the package's own channels in addition.
         */
        'notifications' => [
